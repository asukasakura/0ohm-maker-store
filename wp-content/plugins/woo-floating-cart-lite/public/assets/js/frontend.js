(function( $ ) {
	...
	$(function() {
		...
		var $supports_html5_storage,
			...
			ajaxInit = $$(cartContainer).attr('data-ajax-init') === '1' || !!XT_WOOFC.is_customize_preview,
		...
		function initEvents() {
			...
			if(XT_WOOFC.can_use_premium_code === "1") {
				...
				$$(cartHeader).on(clickSelector, '.xt_woofc-header-clear', function (evt) {
					if ($(evt.target).is($(this))) {
						clearCartConfirm();
					}
				});
				...
				$$(cartHeader).on(clickSelector, '.xt_woofc-header-clear-confirm', function (evt) {
					if ($(evt.target).is($(this))) {
						evt.preventDefault();
						clearCart();
					}
				...
				$$(cartHeader).on(clickSelector, '.xt_woofc-header-clear-cancel', function (evt) {
					if ($(evt.target).is($(this))) {
						evt.preventDefault();
						hideCartNotice();
					}
				...
				$$(cartHeader).on(clickSelector, '.xt_woofc-undo-clear', function (evt) {
					if ($(evt.target).is($(this))) {
						evt.preventDefault();
						clearCartRestore();
					}
				...
				// close cart when clicking on the header close icon
				$$(cartHeader).on(clickSelector, '.xt_woofc-header-close', function (evt) {
					if ($(evt.target).is($(this))) {
						toggleCart(false);
					}
				});
			...
			$$(cartBody).on(clickSelector, '.xt_woofc-delete-item', function(evt){
				...
				var $quantityInput = $(this).closest('li').find('.xt_woofc-quantity-row input');
				var quantity = $quantityInput.length ? $quantityInput.val() : 0;

				var $clone = $(this).clone().addClass('remove').removeClass('xt_woofc-delete-item').hide();
				$(this).parent().append($clone);

				var $wrap = $clone.wrap('<span class="mini_cart_item"></span>').parent();
		...
		function transitionEndClear(el) {
		...
		function clearCartConfirm() {
			...
			setCartNotice(XT_WOOFC.lang.clear_confirm, 'error');
		}
		...
		function clearCartRestore() {

			hideCartNotice(null, true);

			request('clear_restore');
		}
		...
		function toggleCart(flag) {
			...
			var $transitionElement = animationType === 'morph' ? $$(cartWrapper) : $$(cartWrapper);
		...
		function request(type, args, callback) {
			...
			$.XT_Ajax_Queue({
				...
				flushCache();
				console.log(type === 'restore' || 'type' === 'clear_restore');
