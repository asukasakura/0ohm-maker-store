...
<div id="sidebar" class="col-md-4 order-md-first col-sm-12 oder-sm-last">
	...
	?>
		<div id="widget-area" class="widget-area" role="complementary">
			...
			?>
				<span class="edit-link"><a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>" class="badge badge-secondary"><?php esc_html_e( 'Edit', 'test' ); ?></a></span><!-- Show Edit Widget link -->
			<?php
				endif;
			?>
		</div><!-- /.widget-area -->
		...
		<div class="bg-faded sidebar-nav">
			<div id="primary-two" class="widget-area">
					...
					$output = '<ul class="recentposts">';
						...
						if ( $recentposts_query->have_posts() ) :
							...
							while ( $recentposts_query->have_posts() ) :
								...
								$output .= '<h4><a href="' . esc_url( get_the_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'test' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark">' . esc_html( get_the_title() ) . '</a></h4>';
								$output .= '</li>';
							endwhile;
						endif;
						wp_reset_postdata();
				...
				<ul class="categories">
					...
					?>
							<li>&nbsp;</li>
							<li><a href="<?php echo esc_url( get_the_permalink( get_option( 'page_for_posts' ) ) ); ?>" class="btn btn-outline-secondary"><?php esc_html_e( 'more', 'test' ); ?></a></li>
					<?php
						endif;
					?>
				</ul>
